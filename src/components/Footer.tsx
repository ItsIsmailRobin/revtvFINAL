...
export default function Footer() {
  ...
  return (
    <footer className="footer-root px-1 pb-4 pt-3">
      {/* Main row: clock left, credits right — aligned with player */}
      <div className="flex items-center justify-between gap-4">
        {/* Clock pill — no hover, no glow */}
        <div
        ...
        >
          ...
          <span className="text-[14px] font-semibold uppercase tracking-[0.06em] text-white/55">
            TIME
          </span>
          <span className="h-2.5 w-px bg-white/15" />
          <span className="font-mono text-[16px] font-medium tabular-nums text-white/80" style={{ fontVariantNumeric: "tabular-nums" }}>
            {time}
          </span>
          <span className="text-[14px] font-semibold uppercase text-white/55">
            {ampm}
          </span>
        </div>

        {/* Credits — right side, same row */}
        <div className="flex flex-wrap items-center gap-x-3 gap-y-1 justify-end">
          <div className="group flex cursor-default items-center gap-1.5 text-[13px] uppercase tracking-[0.18em] text-white/30 transition-colors duration-300 hover:text-white/70">
          ...
          <p className="flex items-center gap-1.5 text-[13px] uppercase tracking-[0.18em] text-white/25">
